...
import fmat.pgpy.team1.operadores.AdaptadorListPDI;
...
public class ListaMisPDIActivity extends Activity implements ToastInterface, RespuestaInterface{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        customAdapter = new AdaptadorListPDI(contSesion.getSesion().getMisPDI(),this);
    ...
    public void onBorrarPDI(int id){
			    	String usuario=contSesion.getSesion().getCorreo();
	...
	public void procesarRespuestaServidor(JSONObject jObject) {
		...
		try {
			...
			if(tipoRespuesta.equals("100")){
				...
				ArrayList<PuntoDeInteres> misPdis=contSesion.getSesion().getMisPDI();
				Gson gson = new Gson();
				...
				for(int i=0;i<misPdis.size();i++ ){
					if(misPdis.get(i).getId()==pdiEliminado.getId()){
						pdi=misPdis.get(i);
						misPdis.remove(pdi);
						contSesion.getSesion().setMisPDI(misPdis);
